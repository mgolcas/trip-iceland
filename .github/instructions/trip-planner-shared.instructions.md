...
- When a user supplies or requests a route link for a specific hike or object (for example a Komoot route), place the direct link as a short `📱` sub-line immediately below that object's entry in `Dienu_Planas.txt`; keep only the detailed route description and verification notes in `Keliones_Asistentas.txt`, without repeating the same direct URL. Store each piece of information in one canonical location and do not attach a route link to a different day's outing.
- In `Dienu_Planas.txt`, format supporting sub-lines with short symbol-prefixed markers (not plain `•`) so each line is grouped by meaning and can be scanned while navigating.
- Symbol agreement for `Dienu_Planas.txt` sub-lines (use consistently): `⏱️` deadline/critical time, `🕒` check-in/out time window, `🅿️` parking, `🔑` key handover, `🔐` access/self check-in, `💳` payment/refund, `📱` app or online action, `📌` outstanding follow-up, `🔁` fallback/plan B, `⚠️` risk warning, `✅` confirmed status.
- Lodging information agreement: keep the complete practical details for each accommodation (check-in/out rules, late arrival, access, parking, internet, bathroom, kitchen, and key handover) together in the reservation section. In the day schedule, retain only the timed arrival/departure or an action that must happen at that moment; do not duplicate the lodging details.
...
- In `Dienu_Planas.txt`, do not include technical verification math/details (for example `OSRM ... x 1.15`, source lists, or long justification text). Keep only the actionable duration and move verification evidence to `Keliones_Asistentas.txt`.
- If a section in `Dienu_Planas.txt` grows beyond quick-scan use, compress it in the same edit: keep the schedule-critical line, one hard constraint, and at most one fallback note.
...
## Construct KML route names from actual endpoints

Every generated route-line name must show the real place where the leg starts and ends: `N. 🚗 Vairavimas (~XX min, YY km): START → END`. Use the previous and current stop's base place names in itinerary order. A return-to-car action is a stop detail, not a place name: return pins must retain the original parking name plus `(grįžimas prie automobilio)`, and route labels must strip that note so a leg reads `🅿️ Þingvellir P1 – Hakið → 🅿️ Geysir parkingas`, never `🅿️ P1 – grįžimas prie automobilio → ...`. Apply this consistently to every day and transport mode.
